...
var index = function (opt) {
	...
	return {
		...
		onwrite: function onwrite(config, data) {
			var jsList = [];
			var file = fs.readFileSync(template).toString();
			...
			if (dest) {
				...
				for (var i = 0; i < targets.length; i++) {
					if (format === targets[i].format) {
						destPath = path.relative('./', targets[i].dest);
					}
				}
			}
			// has set hash
			if (/\[hash\]/.test(destPath)) {
				...
				fs.writeFileSync(destPath, data.code += "//# sourceMappingURL=" + (path.basename(dest)) + ".map");
			}
			...
			var scripts = jsList.map(function (node) {
				...
				if (node.indexOf(hash) !== -1) {
					return ("<script type=\"text/javascript\" src=\"" + (path.relative(firstDir, node)) + "\"></script>\n");
				}
				return '';
			}).join('');
